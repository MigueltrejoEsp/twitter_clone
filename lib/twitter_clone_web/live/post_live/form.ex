defmodule TwitterCloneWeb.PostLive.Form do
  ...
  defp apply_action(socket, :edit, %{"id" => id}) do
    post = Timeline.get_post!(socket.assigns.current_scope, id)

    socket
    |> assign(:page_title, "Edit Post")
    |> assign(:post, post)
    |> assign(:form, to_form(Timeline.change_post(socket.assigns.current_scope, post)))
  end
  ...
  def handle_event("validate", %{"post" => post_params}, socket) do
    changeset = Timeline.change_post(socket.assigns.current_scope, socket.assigns.post, post_params)
  ...
  defp save_post(socket, :edit, post_params) do
    case Timeline.update_post(socket.assigns.current_scope, socket.assigns.post, post_params) do
      {:ok, post} ->
        {:noreply,
         socket
  ...
  defp save_post(socket, :new, post_params) do
    case Timeline.create_post(socket.assigns.current_scope, post_params) do
      {:ok, post} ->
        {:noreply,
         socket
