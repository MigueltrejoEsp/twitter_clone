defmodule TwitterClone.Timeline do
  ...
  def subscribe_posts(%Scope{} = scope) do
    ...
    Phoenix.PubSub.subscribe(TwitterClone.PubSub, "user:#{key}:posts")
  end

  defp broadcast_post(%Scope{} = scope, message) do
    key = scope.user.id

    Phoenix.PubSub.broadcast(TwitterClone.PubSub, "user:#{key}:posts", message)
  end
  ...
  """
  def list_posts() do
    Repo.all(Post, order_by: [desc: :inserted_at], litmit: 50)
  end
